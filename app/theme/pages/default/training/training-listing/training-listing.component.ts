...
@Component({
    ...
    providers: [TrainingService, MandayService]
})
export class TrainingListingComponent implements OnInit, AfterViewInit {
    ...
    message: any = {
        new: "Bilangan baki sesi latihan : ",
        danger: "Bilangan baki sesi latihan : ",
        success: "Permohonan latihan telah berjaya dihapuskan"
    }
    ...
    constructor(private _script: ScriptLoaderService, private router: Router, private trainingService: TrainingService, private mandayService: MandayService) { }
    ...
    ngAfterViewInit() {
        ...
        var options2 = {
            data: {
                ...
                source: {
                    read: {
                        url: environment.hostname + "/api/trainingTxAudit",
                        headers: {
                            "Authorization": this.bearToken
                        }
                    }
        ...
        let datatable2 = (<any>$('#auditLatihan')).mDatatable(options2);
    ...
    deleteArticle(trainingId: string, title: string) {
        this.confirmMsg = "Adakah anda pasti untuk menghapuskan latihan " + title + " ?";
        this.padamId = trainingId;
        $('#m_modal_padam').modal('show');
    }

    padam() {
        this.trainingService.deleteTrainingById(this.padamId).subscribe(
            success => {
            ...
            errorcode => {
                ...
                $('#m_modal_padam').modal('hide');
                this.confirmMsg = "Latihan tidak berjaya dihapuskan. Pengguna telah memohon latihan tersebut.";
                $('#m_modal_padamUser').modal('show');
            });
    }


    redirectNewTrainingPage() {
        //alert("Bilangan baki manday latihan : " + this.usedManday);
        jQuery('#m_modal_add').modal('hide');
        this.router.navigate(['training/setting']);
    }
