...
export class GradeListComponent implements OnInit, AfterViewInit {
    ...
    ngAfterViewInit() {
        var options = {
            data: {
                ...
                source: {
                    read: {
                        url: environment.hostname + "/api/grade",
                        headers: {
            ...
            columns: [{
                ...
                overflow: "visible",
                template: function(t) {
                    return '<a href="/setting-grade/edit/' + t.id + '" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill btn-outline-success" title="Kemaskini Maklumat">\t\t\t\t\t\t\t<i class="la la-edit"></i>\t\t\t\t\t\t</a>\t\t\t\t\t\t<button id="' + t.id + '" class="deleteFn m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill btn-outline-danger" title="Padam">\t\t\t\t\t\t\t<i class="la la-trash"></i>\t\t\t\t\t\t</button>\t\t\t\t\t'
                }
            }]
        }

        var datatable = (<any>$('#gradeList')).mDatatable(options);
    ...
    redirectNewGradePage() {
        this.router.navigate(['setting-grade/setting']);
    }
